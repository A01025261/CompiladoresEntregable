...
class SemanticVisitor(PTNodeVisitor):
    ...
    def visit_binary(self,node,children):
        ...
        if value >= 2 ** 31: 
            raise SemanticMistake(f'Out of range binary integer literal at position {self.position(node)} => {value}')
    
    def visit_octal(self,node,children):
        value = int(node.value [2:], 8)
        if value >= 2 **31:
            raise SemanticMistake(f'Out of range octal integer literal at position {self.position(node)} => {value}')
        
    def visit_hexadecimal (self,node,children):
        value = int(node.value[2:], 16)
        if value >= 2 ** 31:
            raise SemanticMistake(f'Out of range hexadecimal integer literal at position {self.position(node)} => {value}')
